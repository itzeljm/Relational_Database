...
class CommandInterpreter {
    ...
    void loadStatement() {
        ...
        String name = name();
        _input.next(";");
        Table table1 = Table.readTable(name);
    ...
    void printStatement() {
        ...
        String name2 = name();
        _input.next(";");
        Table table2 = _database.get(name2);

        if (table2 == null) {
            throw error("unknown table: %s", name2);
        } else {
            System.out.println("Contents of " + name2 + ":");
            table2.print();
    ...
    Table selectClause() {
        ...
        if (_input.peek().equals("where")) {
            if (sectab) {
                condlist = conditionClause(a, b);
            } else {
                condlist = conditionClause(a);
            }
        }

        if (sectab) {
            result = a.select(b, colnames, condlist);

        }  else {
            result = a.select(colnames, condlist);
        }
    ...
    ArrayList<Condition> conditionClause(Table... tables) {
        ...
        ArrayList<Condition> returnlist = new ArrayList<Condition>();

        returnlist.add(condition(tables));
    ...
    Condition condition(Table... tables) {
        ...
        Column b = new Column(a, tables);

        String rel = _input.next();

        if (_input.peek().contains("'")) {
            String c = literal();
            return new Condition(b, rel, c);
        } else {
            Column d = new Column(columnName(), tables);
            return new Condition(b, rel, d);
        }
